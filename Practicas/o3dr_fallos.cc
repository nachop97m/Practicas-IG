...
void O3DREVOLUCION::revolucion(int n_puntos = 6, double aini = 30, double afin = 90){
    ...
    float ainir = (aini * 2 * M_PI) / 360;
    float afinr = (afin * 2 * M_PI) / 360;
    ...
    for (; c < finb; c++){
        ...
        this->vertices[c].x = this->vertices[c].x * cos(aini);
        this->vertices[c].z = this->vertices[c].x * sin(aini);

    }

    this->vertices.resize(total);

    float beta = ((afin - aini) * 2 * M_PI)/360;
    float gamma = beta / n_puntos;

    for (int i=0; i < n_puntos; i++){

        for (int c=0; c < size; c++){

            this->vertices[i*size + c].x = this->vertices[c].x * cos(gamma * i);    // Traslacion sobre eje Y
            this->vertices[i*size + c].y = this->vertices[c].y;
            this->vertices[i*size + c].z = this->vertices[c].x * sin(gamma * i);
